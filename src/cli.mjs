...
export async function mcp(){
 ...
 server.setRequestHandler('tools/list',async()=>({tools:Object.entries(schemas).map(([name,properties])=>({name,description:name==='agent_run'?'Run a bounded agent. Requires operator provider opt-in; incurs provider charges and writes private local memory.':'Bounded '+name+'. Generated text grants no execution authority.',inputSchema:{type:'object',properties,required:Object.keys(properties),additionalProperties:false}}))}));
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 try{const[rawAction='help',...rest]=process.argv.slice(2);const action=rawAction.replace(/^agent-/, 'agent_');if(rest.length)throw Error('Arguments use stdin JSON');
  if(action==='mcp')await mcp();else if(action==='help')console.log('Usage: node src/cli.mjs status|templates|template|compile|validate|verify|test|benchmark|agent-plan|agent-run|agent-replay|mcp. Domain arguments use bounded stdin JSON.');
