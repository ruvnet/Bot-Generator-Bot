...
export async function validateProject(){
 ...
 try{return await new Promise((resolve,reject)=>{
  const child=spawn(process.execPath,['--test','test/compiler.test.mjs','test/mcp.test.mjs','test/agent.test.mjs','test/agent-memory.test.mjs','test/agent-provider.test.mjs','test/prompt.test.mjs'],{cwd:root,env:{PATH:process.env.PATH||''},detached:process.platform!=='win32',stdio:['ignore','pipe','pipe']});let size=0,output='',failure;
